...
(function () {
  ...
  function placeholderURI(tones, label) {
    ...
    var text = String(label || "Mon Chérie").slice(0, 34);
    var svg =
  ...
  function cartRead() {
    try {
      ...
      return Array.isArray(arr) ? arr.filter(function (it) { return it && product(it.handle); }) : [];
    } catch (e) { return []; }
  }
  ...
  var cart = {
    ...
    add: function (handle, opts) {
      ...
      var same = items.filter(function (it) {
        return it.handle === handle && it.color === color && it.size === size;
      })[0];
      if (same) same.qty = Math.min(99, same.qty + qty);
      else items.push({ handle: handle, color: color, size: size, qty: qty });
      cartWrite(items);
    ...
    total: function () {
      return cartRead().reduce(function (n, it) {
  ...
  function tplAnnounce() {
    ...
    return '<div class="mc-announce" aria-label="Promoții curente"><div class="mc-announce-track">' + seq + seq + "</div></div>";
  }
    ...
    return '<header class="mc-header">' +
      '<div class="container mc-header-inner">' +
        ...
        '<nav class="mc-nav" aria-label="Navigație principală"><ul>' + tabs +
          '<li><a class="mc-nav-link mc-nav-offers" href="collection.html?c=all">Oferte</a></li>' +
        "</ul></nav>" +
        '<div class="mc-actions">' +
          '<button type="button" class="mc-icon-btn" data-mc-open="search" aria-label="Caută">' + icon("search") + "</button>" +
          '<a class="mc-icon-btn" href="#cont" aria-label="Contul meu">' + icon("user") + "</a>" +
    ...
    return '<div class="mc-drawer" id="mc-drawer" role="dialog" aria-modal="true" aria-label="Meniu">' +
      ...
      '<div class="mc-drawer-viewport">' +
        '<div class="mc-drawer-lvl mc-drawer-lvl1"><ul>' + tabs +
          '<li><a class="mc-drawer-tab mc-nav-offers" href="collection.html?c=all">' + icon("tag") + "<span>Oferte</span></a></li>" +
        "</ul></div>" +
      ...
      '<div class="mc-drawer-foot">' +
        ...
        '<div class="mc-drawer-links">' +
          ...
          '<a href="#cont">' + icon("user") + "Contul meu</a>" +
        "</div>" +
      "</div></div>";
  }
      ...
      '<div class="container">' +
        '<div class="mc-footer-top">' +
          ...
          '<div class="mc-footer-col"><span class="micro">Contact</span><ul class="mc-footer-contact">' +
            ...
            "<li>" + icon("heart") + "<span>salut@moncherie.ro</span></li>" +
            "<li>" + icon("truck") + "<span>Luni–Vineri, 9:00–17:00</span></li>" +
          "</ul></div>" +
        "</div>" +
  ...
  function renderCartUI() {
    ...
    if (nEl) nEl.textContent = count ? "(" + count + (count === 1 ? " produs)" : " produse)") : "";
      ...
      if (!items.length) {
        ...
        list.innerHTML = items.map(function (it, idx) {
          ...
          return '<div class="mc-cart-item" data-tone="' + esc((p.tone || DEFAULT_TONE).join(",")) + '">' +
            ...
            "<div>" +
              '<p class="mc-cart-item-title">' + esc(p.title) + "</p>" +
              ...
              '<div class="mc-cart-item-row">' +
                ...
                "</span>" +
                "<span>" +
                  '<span class="mc-cart-item-price">' + money(p.price * it.qty) + "</span>" +
