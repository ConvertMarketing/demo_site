...
--header-h: 64px;
```
Fonturi (Google Fonts, cu fallback): `Archivo` (display; sans-serif, italic pentru accente) + `Manrope` (UI/body). Fallback: `Helvetica Neue, Arial, sans-serif` / `system-ui, sans-serif`. Diacritice românești obligatorii (subset latin-ext).

Scară tipografică: display `clamp(2.6rem, 6vw, 5rem)` Archivo 340–420 weight, letter-spacing -0.02em; h2 secțiuni `clamp(1.8rem, 3.4vw, 2.6rem)`; micro-etichete 11px uppercase tracking .18em Manrope 600; body 15–16px/1.6.
...
Header: bară anunț sus (marquee lent cu cele 4 promoții reale, pauză la hover) → header sticky (blur backdrop, umbră la scroll): logo „MON CHÉRIE" (Archivo, „chérie" italic), nav desktop = tab-urile din MC_MENU + link „Oferte" (cherry), icons: căutare (overlay cu sugestii din catalog), cont, coș cu badge count.
...
Mobil (<1024px): navigarea trăiește într-un **tab bar fix jos** (fundal `--dark`, colțuri de sus `--radius-lg`, 58px + safe-area): Acasă · Caută · Cont · Coș (badge cherry cu numărul de produse) · Meniu. Tab-ul paginii curente e evidențiat (`aria-current="page"`). Antetul rămâne doar cu logo-ul centrat — iconițele și hamburgerul nu se mai dublează sus. „Meniu" deschide același drawer stânga full-height: nivel 1 = tab-uri cu iconiță + chevron; tap → nivel 2 cu buton „← Înapoi", grupuri + feature-cards ca imagini mici; jos: promo bar + linkuri cont/căutare. Body scroll-lock cât e deschis. Drawerele (z 150) și bottom-sheet-ul de filtre (z 170) acoperă tab bar-ul (z 135); bara sticky de comandă de pe PDP se așază exact deasupra lui, iar `body` primește padding-bottom cât bara.
...
3. **Categorii**: grilă editorială asimetrică (Damă mare + Bărbați, Copii, Treninguri, Cupluri, Accesorii) — imagine, titlu Archivo, „Descoperă →".
...
- Hero compact: breadcrumb, titlu Archivo + sub + desc din MC_COLLECTIONS, count produse.
